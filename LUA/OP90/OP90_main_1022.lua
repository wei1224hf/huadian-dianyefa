...
function HengYi_Vision()
  ...
  --SetAO("request_vision",truss_x.num )
...
function JinLiaoPan_ChaRu()
     if GetAI("tray_in_count") == 0 and GetAI("tray_out_count") == 0 then
  ...
  flag = WaitDI("DI_R_GunTong_DaoWei",1,300000,true)
  --AGV小车将阀块放入滚筒
  while flag do
    Sleep(200)
  ...
  next = GetJointTarget("Xyzuvw")
  next.extax.eax_1 = KGRAB.extax.eax_1
  next.extax.eax_2 = KGRAB.extax.eax_2 - TRAY_HEIGHT* (GetAI("tray_in_count")-1)
  next.extax.eax_3 = KGRAB.extax.eax_2 - TRAY_HEIGHT* (GetAI("tray_out_count")+1)
  MoveAbsJ(next,v200,fine,tool0,wobj0,load0)
end
...
JOINTTARGET("M5_1",{26.272,24.788,69.682,7.491,128.623,78.31,0.000},{9.000,46.373,46.373,0.000,0.000,0.000,0.000})
JOINTTARGET("M5_2",{26.272,24.788,69.682,7.491,128.64,108.51,0.000},{9.000,46.373,46.373,0.000,0.000,0.000,0.000})
JOINTTARGET("M5_3",{26.272,24.788,69.682,7.491,-4.740,106.29,0.000},{9.000,46.373,46.373,0.000,0.000,0.000,0.000})
